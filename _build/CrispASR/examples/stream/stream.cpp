...
int main(int argc, char** argv) {
    ...
    const int n_new_line =
        !use_vad ? std::max(1, params.length_ms / params.step_ms - 1) : 1; // number of steps to print new line
    ...
    while (is_running) {
        ...
        if (!use_vad) {
            ...
            const int n_samples_take =
                std::min((int)pcmf32_old.size(), std::max(0, n_samples_keep + n_samples_len - n_samples_new));
            ...
            // print result;
            {
                ...
                for (int i = 0; i < n_segments; ++i) {
                    ...
                    if (params.no_timestamps) {
                        ...
                        std::string output =
                            "[" + to_timestamp(t0, false) + " --> " + to_timestamp(t1, false) + "]  " + text;
