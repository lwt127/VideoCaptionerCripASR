...
class HuggingFaceDownloadThread(QThread):
    """从HuggingFace下载模型文件的线程"""

    progress = pyqtSignal(int, str)
    finished = pyqtSignal()
    error = pyqtSignal(str)

    def __init__(self, repo_url, files, save_dir):
        super().__init__()
    ...
    def _download_model(self, row):
        ...
        if model.get("modelScopeLink"):
            ...
            self.model_download_thread = HuggingFaceDownloadThread(
                model["downloadLink"],
                model.get("huggingfaceFiles", ["model.bin", "config.json", "tokenizer.json", "vocabulary.json"]),
