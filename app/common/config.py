...
class Config(QConfig):
    ...
    # ------------------- 批量处理配置 -------------------
    # 同时处理的任务数量（并行）。GPU 显存充足时可调高以提升吞吐。
    batch_concurrent_tasks = RangeConfigItem(
        "Batch", "ConcurrentTasks", 2, RangeValidator(1, 8)
    )
    ...
    # 后端引擎、模型、VAD 方法均以标签字符串存储（详见 core/bk_asr/crisp_asr_catalog.py）
