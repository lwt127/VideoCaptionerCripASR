...
def _to_nvenc_preset(preset: str) -> str:
    """Map x264-style preset names to common NVENC preset values."""
...
def _resolve_video_encoder(vcodec: str, use_cuda: bool) -> str:
    """Prefer NVENC for H.264 encode path when CUDA is available."""
    if use_cuda and vcodec == "libx264":
        return "h264_nvenc"
    return vcodec
...
def add_subtitles(
    ...
    if soft_subtitle:
        ...
        encode_vcodec = _resolve_video_encoder(vcodec, use_cuda)
        cmd = ["ffmpeg"]
        if use_cuda:
        ...
        encode_args = []
        if encode_vcodec == "h264_nvenc":
            encode_args.extend(["-cq", "23", "-preset", _to_nvenc_preset(quality)])
        else:
            encode_args.extend(["-preset", quality])
