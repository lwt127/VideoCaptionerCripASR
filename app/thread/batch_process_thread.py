...
class BatchProcessThread(QThread):
    ...
    def __init__(self):
        ...
        # 并行任务数：从配置读取（默认 2），GPU 显存充足时可调高。
        self.max_concurrent_tasks = max(1, int(cfg.batch_concurrent_tasks.value))
        self.is_running = False
        self.factory = TaskFactory()
        self.threads = []  # 保存所有创建的线程

    def set_max_concurrent_tasks(self, value: int):
        """设置并行任务数（由 UI 实时调整）。"""
        self.max_concurrent_tasks = max(1, int(value))
        logger.info(f"并行任务数设为: {self.max_concurrent_tasks}")
