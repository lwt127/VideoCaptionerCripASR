...
from app.core.bk_asr.asr_data import handle_long_path, safe_filename
    ...
    def run(self):
        try:
                ...
                if self.task.need_next_task and self.task.video_path:
                    for subtitle_layout in ["原文在上", "译文在上", "仅原文", "仅译文"]:
                        save_path = str(
                            ...
                            / f"{safe_filename(Path(self.task.video_path).stem)}-{subtitle_layout}.srt"
                        )
                        asr_data.save(
                            save_path=save_path,
            ...
            if self.task.need_next_task and self.task.video_path:
                ...
                video_stem = safe_filename(Path(self.task.video_path).stem)
                save_srt_path = (
                    Path(self.task.video_path).parent / f"{video_stem}.srt"
                )
                asr_data.to_srt(
                    save_path=handle_long_path(str(save_srt_path)),
                    layout=subtitle_config.subtitle_layout,
                )
            else:
                # 删除断句文件（对于仅字幕任务）
